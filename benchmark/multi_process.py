...
def main(name, model_name, batch_size, cache_type, subgraph_bias, cache_percent, dir=None, use_gpu_sampling=False, use_pinned_mem=True, MAX_ITERS=5_000_000, run_profiling=False, trials=1):
    ...
    infer_data = InferenceDataset(
        name, infer_percent, partitions=5, force_reload=False, verbose=True)
    ...
    if name == 'ogbn-papers100M':
        trace = infer_data.create_inference_trace(
            trace_len=1_000_000, subgraph_bias=subgraph_bias)
    else:
    ...
    for trial in range(trials):
        ...
        if cache_type == 'static':
            feat_server = FeatureServer(
                g, 'cuda', ['feat'], use_pinned_mem=use_pinned_mem, profile_hit_rate=True)
        elif cache_type == 'count':
            feat_server = CountingFeatServer(
                g, 'cuda', ['feat'], use_pinned_mem=use_pinned_mem, profile_hit_rate=True)
        elif cache_type == 'lfu':
            feat_server = LFUServer(
                g, 'cuda', ['feat'], use_pinned_mem=use_pinned_mem, profile_hit_rate=True)
        elif cache_type == 'hybrid' or cache_type == 'async':
            feat_server = HybridServer(
                g, 'cuda', ['feat'], use_pinned_mem=use_pinned_mem, profile_hit_rate=True)
        elif cache_type == 'baseline':
            feat_server = None
        elif cache_type == 'cpp':
            feat_server = ManagedCacheServer(
                g, 'cuda', ['feat'], use_pinned_mem=use_pinned_mem, profile_hit_rate=True)
        else:
        ...
        if feat_server:
            ...
            _, indices = torch.topk(out_deg, int(
                g.num_nodes() * cache_percent), sorted=True)
            del out_deg
            ...
            pin_buf = torch.empty(
                (150_000, g.ndata['feat'].shape[1]), dtype=torch.float, pin_memory=True)
        ...
        with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA]) if run_profiling else nullcontext() as prof:
            for i in tqdm(range(0, min(n, MAX_ITERS * BATCH_SIZE), BATCH_SIZE)):
                ...
                with Timer(name='total'):
                    ...
                    mfgs = sampler.sample(
                        trace.nids[i:i+BATCH_SIZE], trace.edges[i:i+BATCH_SIZE], use_gpu_sampling=use_gpu_sampling)
                    ...
                    with Timer(name="dataloading", track_cuda=True):
                        ...
                        if feat_server:
                            ...
                            inputs, mfgs = feat_server.get_features(
                            ...
                            if use_pinned_mem:
                                with Timer('resize pin buf'):
                                    pin_buf = pin_buf.resize_(
                                        (required_feats.shape[0], pin_buf.shape[1]))
                                inputs = pin_buf.narrow(
                                    0, 0, required_feats.shape[0])

                            # Baseline: feature gather + CPU-GPU copy
                            with Timer('feature gather'):
                                if use_pinned_mem:
                                    torch.index_select(
                                        g.ndata['feat'], 0, required_feats, out=inputs)
                                else:
        ...
        if run_profiling:
            prof.export_chrome_trace(
                f"trace_{model_name}_{name}_{batch_size}_{cache_type}{'_bias_0.8' if subgraph_bias is not None else ''}{'_pinned' if use_pinned_mem else ''}.json")
        print_timer_info()
        if dir != None:
            ...
            trial_dir = os.path.join(
                trial_dir, f'bias_{subgraph_bias}' if subgraph_bias is not None else 'uniform')

            if cache_type == 'baseline':
                trial_dir = os.path.join(trial_dir, cache_type)
            else:
                trial_dir = os.path.join(
                    trial_dir, f'{cache_type}_{cache_percent}')

            export_timer_info(f'{trial_dir}/{model_name.upper()}',
                              {'name': name, 'batch_size': batch_size, 'trial': trial})
            if feat_server:
                feat_server.export_profile(f'{trial_dir}/{model_name.upper()}_cache_info', {
                                           'name': name, 'batch_size': batch_size, 'trial': trial})


class CUDATest(Process):
    def __init__(self, t, id):
        super().__init__()
        self.t = t
        self.id = id
        print(id, 'started with', t)
