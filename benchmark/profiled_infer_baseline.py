...
def main():
    ...
    with record_function("create_gcn"):
        in_size = g.node_attr_schemes()['feat'].shape[0]
        out_size = infer_data.num_classes
        # Model goes on DEVICE
        model = GCN(in_size, 16, out_size).to(device)
        model.eval()

    with record_function("print_model"):
        print(g)

    # Test static batch size of 1
    n = infer_data.trace_len // 50
    ...
    for i in tqdm(range(n)):
        with Timer(name='total'):
            ...
            with record_function("CPU-GPU copy"):
